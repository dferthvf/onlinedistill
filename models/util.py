...
def create_model(name, n_cls, dataset='miniImageNet'):
    ...
    if dataset == 'miniImageNet' or dataset == 'tieredImageNet':
        if name.endswith('v2') or name.endswith('v3'):
            ...
            # model = model_dict[name](avg_pool=True, drop_rate=0.0, dropblock_size=5, num_classes=n_cls)
            model = model_dict[name](avg_pool=True, drop_rate=0.1, dropblock_size=5, num_classes=n_cls, use_se=False)
