...
class JigsawCrop(object):
    """Jigsaw style crop"""
    def __init__(self, n_grid=3, img_size=84, crop_size=28):
        self.n_grid = n_grid
...
class ImageNet(Dataset):
    def __init__(self, args, partition='train', pretrain=True, is_sample=False, k=4096,
        ...
        if self.pretrain or self.partition == 'train':
    ...
    def __getitem__(self, item):
        ...
        img_jigsaw = self.jigsaw_transform(img)

        if not self.is_sample:
            return img_view1, img_view2, img_jigsaw, target, item
        else:
            pos_idx = item
            replace = True if self.k > len(self.cls_negative[target]) else False
            neg_idx = np.random.choice(self.cls_negative[target], self.k, replace=replace)
            sample_idx = np.hstack((np.asarray([pos_idx]), neg_idx))
            return img_view1, img_view2, img_jigsaw, target, item, sample_idx
...
class MetaImageNet(ImageNet):
    ...
    def __init__(self, args, partition='train', train_transform=None, test_transform=None, fix_seed=True):
        ...
        self.n_test_runs = args.n_train_runs if partition == 'train' else args.n_test_runs
